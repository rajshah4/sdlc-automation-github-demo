...
def test_search_pets_combines_age_with_species_filter() -> None:
    results = search_pets(species="dog", min_age_months=15)

    assert [pet.name for pet in results] == ["Scout"]


def test_search_pets_age_filter_preserves_default_status() -> None:
...
def test_search_pets_age_boundary_minimum_inclusive() -> None:
    results = search_pets(min_age_months=18)

    assert "Mochi" in [pet.name for pet in results]


def test_search_pets_age_boundary_maximum_inclusive() -> None:
