...
def test_search_pets_filters_by_min_age() -> None:
    results = search_pets(min_age_months=20)
...
def test_search_pets_rejects_inverted_age_range() -> None:
    with pytest.raises(
        ValueError, match="min_age_months cannot be greater than max_age_months"
    ):
        search_pets(min_age_months=30, max_age_months=10)


def test_search_pets_age_filter_with_status() -> None:
