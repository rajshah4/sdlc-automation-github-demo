...
def test_search_pets_filters_by_species_and_age_range() -> None:
    results = search_pets(species="dog", min_age_months=10, max_age_months=30)

    assert [pet.name for pet in results] == ["Scout"]


def test_search_pets_age_filter_respects_default_available_status() -> None:
    results = search_pets(min_age_months=10, max_age_months=20)
...
def test_search_pets_age_filter_works_with_pending_status() -> None:
...
def test_search_pets_accepts_equal_min_max_age() -> None:
    results = search_pets(min_age_months=18, max_age_months=18)

    assert [pet.name for pet in results] == ["Mochi"]


def test_search_pets_accepts_zero_age() -> None:
