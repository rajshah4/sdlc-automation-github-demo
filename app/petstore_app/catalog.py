...
def search_pets(
    ...
    *,
    ...
    tag: str | None = None,
    max_results: int = 10,
    max_adoption_fee_cents: int | None = None,
) -> list[Pet]:
    """Search pets by name, species, status, tag, and optional maximum adoption fee."""
    if max_results < 1 or max_results > 50:
        raise ValueError("max_results must be between 1 and 50")
    if max_adoption_fee_cents is not None and max_adoption_fee_cents < 0:
        raise ValueError("max_adoption_fee_cents cannot be negative")
    ...
    for pet in PETS:
        ...
        if normalized_tag and normalized_tag not in pet.tags:
            continue
        if max_adoption_fee_cents is not None and pet.adoption_fee_cents > max_adoption_fee_cents:
            continue
        matches.append(pet)
