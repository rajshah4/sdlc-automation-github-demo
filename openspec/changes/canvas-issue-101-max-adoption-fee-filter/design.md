...
- Add filter predicate immediately before `matches.append(pet)`:
...
- Wire a `change` listener on `#max-fee` mirroring the existing `#family-friendly` listener.
...
|------|----------|------------|
| Fee format mismatch: backend cents vs UI dollar string | Medium | `feeToCents()` helper isolates the conversion; unit tested implicitly via UI filter behavior |
| Negative fee input from user | Low | HTML `min="0"` prevents browser-side negatives; backend raises `ValueError` on API calls |
